...
uint8_t *mmap_file(char *filename, int *fd) {
    ...
    /* Step 1: If filename isn't an absolute pathname, then we'd better prepend
...
struct bpb33 *check_bootsector(uint8_t *image_buf) {
    ...
    if (bootsect->bsJump[0] == 0xe9 || (bootsect->bsJump[0] == 0xeb && bootsect->bsJump[2] == 0x90)) {
#ifdef DEBUG
...
uint16_t get_fat_entry(uint16_t clusternum, uint8_t *image_buf, struct bpb33 *bpb) {
    ...
    /* this involves some really ugly bit shifting.  This probably only works on a little-endian machine. */
...
/* is_end_of_file returns true if the FAT entry for cluster indicates this is the last cluster in a file */
...
/* root_dir_addr returns the address in the mapped disk image for the
...
/* cluster_to_addr returns the memory location where the memory mapped cluster actually starts */
